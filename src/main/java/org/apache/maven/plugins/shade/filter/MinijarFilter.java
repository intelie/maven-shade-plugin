...
public class MinijarFilter
    ...
    private void removeSpecificallyIncludedClasses(MavenProject project, List<SimpleFilter> simpleFilters)
        ...
        for (Artifact dependency : project.getArtifacts())
            addDependencyToClasspath(checkCp, dependency);

        Set<Clazz> toInclude = new HashSet<Clazz>();
        for (Artifact dependency : project.getArtifacts()) {
            ...
            for (SimpleFilter simpleFilter : simpleFilters) {
                if (simpleFilter.canFilter(jar)) {
                    ...
                    if (depClazzpathUnit != null) {
                        for (Clazz clazz : depClazzpathUnit.getClazzes()) {
                            if (simpleFilter.isSpecificallyIncluded(clazz.getName().replace('.', '/'))) {
                                toInclude.add(clazz);
                                toInclude.addAll(clazz.getTransitiveDependencies());
                            }
                        }
                    }
                }
